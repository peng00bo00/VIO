...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    Eigen::Quaterniond last_Qwb(init_Rwb_);
    MotionData last_imupose = imudata[0];
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        Eigen::Vector3d dtheta_half =  0.5*(imupose.imu_gyro * dt /2.0 + last_imupose.imu_gyro * dt/2.0);
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        /// 中值积分
        Qwb = last_Qwb * dq;
        Eigen::Vector3d acc_w = 0.5*(Qwb * (imupose.imu_acc) + gw + last_Qwb * (last_imupose.imu_acc) + gw);
        Vw = Vw + acc_w * dt;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;

        last_Qwb = Qwb;
        last_imupose = imupose;
